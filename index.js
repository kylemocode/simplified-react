...
function render(element, container) {
  /*
    Old and bad method without fiber:
...
/* requestIdleCallback：每個 frame 的最後如果 main thread idle 就會執行 callback，react 現在
不用這個方式，而是改用 scheduler，不過概念類似，是 concurrent mode 可以實現的基礎 */
...
function performUnitOfWork(fiber) {
  ...
  */
  ...
  // find the next unit of work. child first, then sibling, then uncle.
  if (fiber.child) {
    return fiber.child;
  }
  let nextFiber = fiber;
...
/* 
Fibers: organize units of work. One fiber for each element and each fiber will be unit of work 
root fiber -> set to nextUnitOfWork
*/
